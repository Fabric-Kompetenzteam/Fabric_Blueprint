# Fabric notebook source

# METADATA ********************

# META {
# META   "kernel_info": {
# META     "name": "synapse_pyspark"
# META   },
# META   "dependencies": {
# META     "lakehouse": {
# META       "default_lakehouse": "b52127d0-485c-4fb0-b7ee-b8d80858f144",
# META       "default_lakehouse_name": "Blueprint_LH",
# META       "default_lakehouse_workspace_id": "1d8f6de1-70b1-4989-ac36-66143ee32cfd",
# META       "known_lakehouses": [
# META         {
# META           "id": "b52127d0-485c-4fb0-b7ee-b8d80858f144"
# META         }
# META       ]
# META     },
# META     "environment": {}
# META   }
# META }

# MARKDOWN ********************

# #### Creating Stage Table

# CELL ********************

# MAGIC %%sql
# MAGIC CREATE SCHEMA IF NOT EXISTS stage

# METADATA ********************

# META {
# META   "language": "sparksql",
# META   "language_group": "synapse_pyspark"
# META }

# MARKDOWN ********************

# ###### Schema Stage Table

# CELL ********************

# MAGIC %%sql
# MAGIC CREATE TABLE IF NOT EXISTS stage.salesorder (
# MAGIC     SalesOrderNumber            STRING,
# MAGIC     SalesOrderLineNumber        INT,
# MAGIC     OrderDate                   DATE,
# MAGIC     CustomerName                STRING,
# MAGIC     Email                       STRING,
# MAGIC     Item                        STRING,
# MAGIC     Quantity                    INT,
# MAGIC     UnitPrice                   DECIMAL(18, 2),
# MAGIC     Tax                         DECIMAL(18, 2),
# MAGIC     IsFlagged                   BOOLEAN,
# MAGIC     __SourceFile                STRING,
# MAGIC     __InsertTimestampStageUTC   TIMESTAMP,
# MAGIC     __ModifiedTimestampStageUTC TIMESTAMP
# MAGIC )

# METADATA ********************

# META {
# META   "language": "sparksql",
# META   "language_group": "synapse_pyspark"
# META }
